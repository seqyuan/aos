...
// cmdRestore annotos restore：按任务记录还原软链接。
// 对任务中的每条软链接记录，在目标目录下重建同名 symlink 指向记录的 link_target。
func cmdRestore(args []string) int {
	fs := flag.NewFlagSet("annotos restore", flag.ContinueOnError)
	...
	if ok, err := parseFlagSet(fs, args, "用法: annotos restore -id <任务ID> [-d <目录>] [-f]\n\n示例:\n  annotos restore -id 3 -d /local/matrix    # 在 /local/matrix 下还原任务 3 的软链接"); !ok {
		return 2
	} else if err != nil {
		return 2
	}
	if *taskID <= 0 {
		fmt.Fprintln(os.Stderr, "annotos restore: 需要 -id <任务ID>（用 annotos stat 查看）")
		fs.Usage()
		return 2
	}
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "annotos restore: 无法打开数据库 %s: %v\n", path, err)
		return 1
	}
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "annotos restore: %v\n", err)
		return 1
	}
	...
	for _, l := range links {
		dest := filepath.Join(root, filepath.FromSlash(l.LinkRel))
