...
- **新功能：`cp` 上传支持 `--skip-existing`**——跳过云端已存在且内容一致的对象，断线后重跑不再重复上传
  - 先列云端目标前缀，对每个待传文件比对：云端无同 key → 传；同 key 但大小不同 → 传；大小相同则本地用 crc64.ECMA 复算与云端 crc64 比对，一致 → 跳过
  - 云端无 crc64 指纹（老对象等）时保守上传，不做误跳过；本地文件读取失败同样保守上传
  - 被跳过的文件不记录任务库、不进进度；全部跳过时直接提示完成
  - 软链接转文本文件（默认模式）按文本内容比对；溯源上传（`--follow-links`）按真实内容比对
  - 与 `--checkpoint`（中断分片续传）互补：前者跳过已完成的整个对象，后者续传中断的大文件
